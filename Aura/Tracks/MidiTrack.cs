...
public sealed class MidiTrack : Track
{
    ...
    public override void StartRecording(string destPath)
    {
        ...
        if (AuraMain.MidiDevice == null || AuraMain.MidiDevice.InDevice == null)
        {
            throw new Exception("To start recording select a midi input device with: AuraMain.CreateMidiDevice().");
        }
        _lastRecordPath = destPath;
        _recInstance = new Recording(TempoMap.Default, AuraMain.MidiDevice.InDevice);
